...
contract CellarAaveV3Test is MainnetStarterTest, AdaptorHelperFunctions {
    ...
    function setUp() external {
        ...
        cellar.addAdaptorToCatalogue(address(aaveATokenManagerAdaptor));
        cellar.addAdaptorToCatalogue(address(aaveDebtManagerAdaptor));
    ...
    function testDeposit(uint256 assets) external {
        ...
        assertApproxEqAbs(
            ...
            assets + initialAssets,
            1,
            "Assets should be added to the cellar's total assets."
        );
    }
    ...
    function testWithdrawalLogicEModeWithDebt() external {
        ...
        data[1] = Cellar.AdaptorCall({ adaptor: address(aaveDebtManagerAdaptor), callData: adaptorCalls1 });
        ...
        data[2] = Cellar.AdaptorCall({ adaptor: address(aaveATokenManagerAdaptor), callData: adaptorCalls2 });
        cellar.callOnAdaptor(data);
    ...
    function testWithdrawableFromaV3WETH() external {
        ...
        data[2] = Cellar.AdaptorCall({
            adaptor: address(aaveDebtManagerAdaptor),
            callData: adaptorCallsForThirdAdaptor
        });
        cellar.callOnAdaptor(data);
    ...
    function testTakingOutAFlashLoan() external {
        ...
        data[0] = Cellar.AdaptorCall({ adaptor: address(aaveDebtManagerAdaptor), callData: adaptorCallsForFlashLoan });
        cellar.callOnAdaptor(data);
    ...
    function testMultipleATokensAndDebtTokens() external {
        ...
        data[2] = Cellar.AdaptorCall({
            adaptor: address(aaveATokenManagerAdaptor),
            callData: adaptorCallsThirdAdaptor
        });
        data[3] = Cellar.AdaptorCall({ adaptor: address(aaveDebtManagerAdaptor), callData: adaptorCallsFourthAdaptor });
        cellar.callOnAdaptor(data);
    ...
    function testIntegration() external {
...

        {
            ...
            data[2] = Cellar.AdaptorCall({ adaptor: address(aaveATokenManagerAdaptor), callData: adaptorCalls });
        }
        ...
        {
            uint256 cellarAV3WETH = aV3WETH.balanceOf(_getAccountAddress(DEFAULT_ACCOUNT));
        ...
        // Create data to lend remaining USDC on Aave.
        {
            ...
            data[1] = Cellar.AdaptorCall({ adaptor: address(aaveATokenManagerAdaptor), callData: adaptorCalls });
        }
    ...
    function _getAccountAddress(uint8 accountId) internal view returns (address) {
        ...
        return Create2.computeAddress(salt, bytecodeHash, address(cellar));
    }
}
